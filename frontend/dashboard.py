...
def dashboard_page():
    ...
    with header_col1:
        ...
        st.caption(
            "This is your central hub to manage resumes, analysis, and job recommendations."
        )
    with header_col2:
    ...
    if section == "Overview":
        ...
        with st.container(border=True):
            ...
            if resume_row:
                st.write(
                    f"- 📄 Last resume uploaded: **{file_name}** at **{uploaded_at}**"
                )
                st.write(
                    "- 📊 Resume analysis not yet implemented (will be added in next milestone)."
                )
                st.write("- 💼 Job recommendations feature coming soon.")
            else:
                st.write(
                    "- No recent activity yet. Upload your first resume to get started."
                )
        ...
        with st.container(border=True):
            ...
            with c1:
                st.info(
                    "📄 **Upload your resume** to enable analysis and job recommendations."
                )
            ...
            with c2:
                st.info(
                    "🤖 **Resume Analysis** will use AI to score and improve your resume."
                )
    ...
    # ============================
        ...
        with st.container(border=True):
            ...
            st.info(
                "In future, you can add options here to edit profile, change password, etc."
            )
                ...
                if uploaded_file.size > MAX_FILE_SIZE:
                    ...
                    if st.button("Save & Extract Text"):
                        ...
                        if success:
                            ...
                            st.error(
                                msg
                                + "  \n\n👉 Please upload a *digital* resume "
                                "(created in Word/Google Docs/Canva) instead of a "
                                "scanned photo PDF for best results."
                            )
            else:
    ...
    # ============================
        ...
        with st.container(border=True):
            ...
            if resume_row:
                st.write(
                    f"Latest stored resume: **{file_name}** (uploaded at {uploaded_at})"
                )
            ...
            st.caption(
                "More settings (notification preferences, theme, etc.) can be added later."
            )