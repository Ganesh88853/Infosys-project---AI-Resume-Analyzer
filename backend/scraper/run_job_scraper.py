...
def run_scraper(max_pages=3):
    ...
    try:
        ...
        # 2️⃣ LOAD USER PREFERENCES (TASK 19)
        ...
        for location in locations:
            ...
            for link in job_links:
                ...
                job_id = result  # save_job should return job_id

                # 7️⃣ EXTRACT JOB REQUIREMENTS (LLM)
                job_req = extract_job_requirements(job["description"])

                # 8️⃣ CALCULATE MATCH SCORE
                match_score = calculate_overall_match(
                    user_profile,
                    job_req
                )

                # 9️⃣ GENERATE EXPLANATION
